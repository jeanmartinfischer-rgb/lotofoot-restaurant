...
export default async function Admin({ searchParams }: { searchParams: { msg?: string } }) {
  ...
  async function resetPassword(formData: FormData) {
    ...
    const newPwd = ((formData.get('newpwd') as string) ?? '').trim();
    if (newPwd.length < 6) {
      redirect('/admin?msg=' + encodeURIComponent('Mot de passe trop court (min 6 caracteres)'));
    }
    ...
    <div className="space-y-4">
      ...
      <section>
        ...
        <ul className="space-y-2">
          {users?.map((u: any) => (
            <li key={u.id} className="rounded-2xl border border-ligne bg-ardoise p-3 text-sm space-y-2">
              ...
              <form action={resetPassword} className="flex gap-1 border-t border-ligne pt-2">
                <input type="hidden" name="id" value={u.id} />
                <input type="hidden" name="pseudo" value={u.pseudo} />
                <input name="newpwd" type="text" placeholder="Nouveau mot de passe" className="flex-1 rounded-lg border border-ligne bg-pitch px-3 py-1.5 text-xs text-chalk outline-none focus:border-sang-vif" />
                <button className="rounded-lg bg-sang border border-sang px-3 py-1.5 text-xs font-semibold text-chalk whitespace-nowrap">
                  Reinitialiser
                </button>
              </form>
              {!u.is_admin && (
