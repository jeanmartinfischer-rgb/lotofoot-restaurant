...
const LINK = "flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-lg text-[10px] font-semibold text-chalk/70 hover:text-chalk hover:bg-ardoise whitespace-nowrap transition-colors md:flex-1";

export default async function RootLayout({ children }: { children: React.ReactNode }) {
  // Detecter la page courante : on masque le menu sur les pages d'authentification
  let pathname = '';
  try {
    const h = headers();
    pathname = h.get('x-pathname') ?? h.get('x-invoke-path') ?? h.get('referer') ?? '';
  } catch {
    pathname = '';
  }
  const isAuthPage =
      ...
      <body className="min-h-dvh">
        ...
        {!isAuthPage && (
          <header className="sticky top-0 z-20 border-b border-ligne bg-pitch/95 backdrop-blur">
            ...
            <div className="relative border-t border-ligne">
              {/* Voiles lateraux : suggerent le scroll sur mobile, masques sur ordinateur */}
              <div className="pointer-events-none absolute left-0 top-0 bottom-0 w-6 bg-gradient-to-r from-pitch to-transparent md:hidden z-10" />
              <div className="pointer-events-none absolute right-0 top-0 bottom-0 w-6 bg-gradient-to-l from-pitch to-transparent md:hidden z-10" />
              <nav className="overflow-x-auto scrollbar-hide">
                <div className="flex min-w-max md:min-w-0 md:w-full px-2 py-1 gap-1" style={{ color: '#D4AF37' }}>
